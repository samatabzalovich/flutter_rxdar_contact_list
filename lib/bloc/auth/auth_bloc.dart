...
class AuthBloc {
  ...
  factory AuthBloc() {
    ...
    final authStatus = FirebaseAuth.instance.authStateChanges().map((user) {
      if (user == null) {
    ...
    final  Stream<AuthError?> loginError = login.setLoadingTo(true, onSink: isLoading).asyncMap<AuthError?>((loginCommand) async {
      try {
